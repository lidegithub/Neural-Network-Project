...
public class FFNNHyperbolic {
    ...
    public FFNNHyperbolic(int I, int J, int K, double eta, double alpha) {
    ...
    private double[] calculateGammaO(double[] target, double[] output) {
        ...
        for (int k = 0; k < target.length; k++) {
            gammaO[k] = -1*(target[k] - output[k]) * hyperbolicDerivate(output[k]);
        }
        return gammaO;
    }
    ...
    private double[] calculateGammaY(double[] gammaO, double[] y, double[][] w) {
        ...
        for (int j = 0; j < w[0].length; j++) {
            for (int k = 0; k < w.length; k++) {
                gammaY[j] += gammaO[k] * w[k][j] * hyperbolicDerivate(y[j]);
            }
        }
        return gammaY;
    }
    ...
    private double[] calculateNonLinearVector(double[] sup) {
        ...
        for (int i = 0; i < sup.length; i++) {
            nonLinearVector[i] = hyperbolic(sup[i]);
        }
        return nonLinearVector;

    }

    private double hyperbolic(double x) {
        double f = 0;
        f = (2/(1+Math.exp(-1*x))) - 1;
        return f;
    }

    private double hyperbolicDerivate(double x) {
        double f = 0;
        f = (1+x)*(1-x)*0.5;
        return f;
    }
